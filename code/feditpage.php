...
{	
	...
	if(@mysql_connect('localhost','root',''))
	{
		...
		{
			$query_run=mysql_query('SELECT * FROM tbl_faculty WHERE register= "'.$_SESSION['user'].'" ');
			...
			if(isset($_POST['submit']))
				{	if($_POST['password'])
						{
							...
							{	
								if($fileupdate = mysql_query("UPDATE tbl_faculty SET name='".$_POST['username']."',email='".$_POST['email']."',phone='".$_POST['number']."',link='".$_POST['link']."',about='".$_POST['txt']."',password='".md5($pass)."'  WHERE register='".$_SESSION['user']."'" ))
								header('location:userprofile.php');
							}	
							else
							{
			...
			else if (isset($_POST['submitproject'])) 
				{
					...
					{	
						...
						$project+=1;
						if($projcount=mysql_query("UPDATE tbl_faculty SET projects='".$project."'"))
			...
			else if (isset($_POST['submitpublication'])) 
				{
					...
					{
						$certifications+=1;
						if($projcount2=mysql_query("UPDATE tbl_faculty SET certifications='".$certifications."'"))
						header('location:fuserprofile.php');
				}
				}
			else if (isset($_POST['submitcert'])) 
				{
					...
					{ $publications+=1;
						if($projcount3=mysql_query("UPDATE tbl_faculty SET publications='".$publications."'"))
						header('location:fuserprofile.php');
				}
				}
		}
